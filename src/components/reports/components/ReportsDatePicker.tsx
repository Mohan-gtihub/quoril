    ...
    return (
        <div className="flex-1 min-w-[220px]">
            ...
            <div className="grid grid-cols-7 mb-1">
                {WEEK_DAYS.map(d => (
                    <div key={d} className="text-center text-[11px] font-bold text-white/20 py-1">{d}</div>
                ))}
            </div>
            {/* Days grid */}
                    ...
                    return (
                        <button
                            ...
                            className={`
                                ...
                                ${inR && !isEdge ? 'bg-[var(--accent-lime-100)] text-white/80 rounded-none' : ''}
                                ${isEdge ? 'bg-[var(--accent-primary)] text-[var(--accent-contrast)] rounded-lg z-10' : 'text-white/60 hover:text-white hover:bg-white/[0.06] rounded-lg'}
                            `}
                        >
                            {format(day, 'd')}
                            {isToday && !isEdge && (
...
export function DateRangePicker({ value, onChange }: Props) {
    ...
    return (
        <div className="relative" ref={ref}>
            ...
            <button
                onClick={() => setOpen(o => !o)}
                className="flex items-center gap-2 px-3 py-2 rounded-xl bg-white/[0.04] border border-white/[0.08] hover:bg-white/[0.07] hover:border-[var(--accent-primary)]/40 transition-all text-sm font-semibold text-white/70"
            >
                <Calendar className="w-3.5 h-3.5 text-[var(--accent-primary)] flex-shrink-0" />
                        ...
                        <div className="w-36 border-r border-white/[0.07] py-3 flex flex-col gap-0.5">
                            {presets.map(p => (
                                <button
                                    ...
                                    className={`w-full text-left text-xs px-4 py-2 font-medium transition-all hover:text-white ${activePreset === p.label
                                        ? 'bg-[var(--accent-lime-100)] text-[var(--accent-primary)] font-bold'
                            ...
                            <button
                                ...
                                className={`w-full text-left text-xs px-4 py-2 font-medium transition-all hover:text-white ${activePreset === 'Custom'
                                    ? 'bg-[var(--accent-lime-100)] text-[var(--accent-primary)] font-bold border-t border-white/[0.07] mt-0.5'
                        ...
                        <div className="flex flex-col">
                            ...
                            <div className="flex items-center gap-3 px-5 py-3 border-t border-white/[0.07] bg-white/[0.02]">
                                ...
                                <button
                                    ...
                                    className="px-5 py-1.5 rounded-lg bg-[var(--accent-primary)] hover:bg-[var(--accent-primary)] disabled:opacity-30 text-[var(--accent-contrast)] text-xs font-bold transition-all shadow-lg shadow-[var(--accent-glow)]"
                                >
                                    Apply
                                </button>
                            </div>
