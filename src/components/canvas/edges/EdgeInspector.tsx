...
export function EdgeInspector({ edgeId, onClose }: { edgeId: string; onClose: () => void }) {
    ...
    return (
        ...
        >
            ...
            <div>
                <div className="text-[11px] uppercase tracking-wider text-[var(--text-muted)] mb-1">Type</div>
                <div className="grid grid-cols-3 gap-1">
                    {KINDS.map((k) => (
                        <button key={k.value} onClick={() => setKind(k.value)}
                            className={`text-[11px] px-2 py-1.5 rounded border ${conn.kind === k.value ? 'border-[var(--accent-primary)] bg-[var(--accent-primary)]/10' : 'border-[var(--border-default)] hover:bg-[var(--bg-hover)]'}`}>
                            <div className="font-medium text-[var(--text-primary)]">{k.label}</div>
                            <div className="text-[11px] text-[var(--text-muted)]">{k.desc}</div>
                        </button>
                    ))}
                </div>
            </div>
            ...
            <div className="flex items-center gap-2">
                ...
                <label className="text-[11px] text-[var(--text-muted)] flex items-center gap-1 ml-auto">
                    Width
            ...
            <div className="flex gap-2">
                ...
                <button
                    ...
                    className={`flex-1 text-xs px-2 py-1.5 rounded ${conn.condition ? 'bg-[var(--accent-primary)] text-[var(--accent-contrast)]' : 'bg-[var(--bg-elevated)] hover:bg-[var(--bg-hover)] text-[var(--text-primary)]'}`}
            ...
            {conn.condition && (
                <div className="text-[11px] text-[var(--text-muted)] p-2 bg-[var(--bg-elevated)] rounded">
